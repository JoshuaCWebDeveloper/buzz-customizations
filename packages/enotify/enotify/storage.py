...
class StoreTransaction:
    def __init__(self, store: "Store", db: sqlite3.Connection):
    ...
    def record_occurrence(self, occurrence: EventOccurrence) -> dict[str, Any]:
        return self.store._record_occurrence(self.db, occurrence)

    def checkpoint(self, provider: str, source: str, cursor: str) -> None:
        self.db.execute(
            """INSERT INTO provider_checkpoints(provider,source,cursor,updated_at) VALUES(?,?,?,?)
            ...
            (provider, source, cursor, now()),
        )


class Store:
    def __init__(self, path: Path, migrations: Path | None = None, extensions: dict[tuple[str, str], Any] | None = None):
        self.path = Path(path)
        self.migrations = migrations or Path(__file__).parents[1] / "migrations"
        self.db: sqlite3.Connection | None = None
        if extensions is None:
            from .storage_extensions import default_extensions
    ...
    def transaction(self) -> Iterator[StoreTransaction]:
        with self._transaction() as db:
            yield StoreTransaction(self, db)

    def _extension(self, event: Any) -> Any | None:
        return self.extensions.get((event.provider, event.event_type))
    ...
    def create(
        ...
        with self._transaction() as db:
            ...
            if extension is not None:
                extension.on_create(StoreTransaction(self, db), subscription_id, event, stamp)
    ...
    def update(
        ...
        with self._transaction() as db:
            ...
            extension = self._extension(event or EventTriggerSpec.from_mapping(old["event_trigger"]))
            if extension is not None:
                extension.on_update(StoreTransaction(self, db), old, event, expected + 1)
    ...
    def transition(self, subscription_id: str, action: str, expected: int) -> dict[str, Any]:
        ...
        with self._transaction() as db:
            ...
            event_row = db.execute("SELECT event_json FROM subscriptions WHERE id=?", (subscription_id,)).fetchone()
            if event_row:
                event = EventTriggerSpec.from_mapping(json.loads(event_row[0]))
                extension = self._extension(event)
                if extension is not None:
                    extension.on_transition(StoreTransaction(self, db), {**self.get(subscription_id)}, action, expected + 1)
    ...
    def _record_occurrence(self, db: sqlite3.Connection, occurrence: EventOccurrence) -> dict[str, Any]:
        row_id = str(uuid.uuid4())
        db.execute(
            """INSERT OR IGNORE INTO event_occurrences VALUES(?,?,?,?,?,?,?,?)""",
            (row_id, occurrence.provider, occurrence.source, occurrence.occurrence_id,
             occurrence.observed_at, occurrence.cursor, canonical_json(occurrence.payload or {}), now()),
        )
        row = db.execute(
    ...
    def record_occurrence(self, occurrence: EventOccurrence) -> dict[str, Any]:
        with self._transaction() as db:
            return self._record_occurrence(db, occurrence)
