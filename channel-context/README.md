...
This independent customization injects per-channel context into Codex turns that are framed by Buzz. The hook reads the `prompt` field from Codex's `UserPromptSubmit` JSON input, accepts a channel only from a `Channel: ... (<UUID>)` Buzz context line, and concatenates regular files in `$CODEX_HOME/channel-context/<UUID>/` in filename order.
...
Use `--codex-home PATH` for a staging home or an explicitly selected user home. Installation updates only the `UserPromptSubmit` array, preserves unrelated JSON, and writes `hooks.json.buzz-customizations-backup` before replacement. It marks its own group so repeated installs replace only the customization's previous group.
